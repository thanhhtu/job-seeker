...
class JobSchema(BaseModel):
    ...
    def normalize_job_level(cls, v: Any, info: Any) -> Optional[str]:
        # Lấy từ 'job_expertise' (key của input mẫu) hoặc từ nested fields
        val = v
        if not val:
    ...
    def map_industry(cls, v: Any, info: Any) -> Optional[str]:
        # Lấy từ các key tiềm năng
        return v or info.data.get("company_industry") or info.data.get("company_type")

if __name__ == "__main__":
    # Load data from JSON file
    data_file = Path(__file__).parent.parent / "data" / "itviec_jobs.json"
    output_file = Path(__file__).parent.parent / "data" / "itviec_jobs_schema.json"
